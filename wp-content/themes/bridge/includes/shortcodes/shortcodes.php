        ...
        if($icon != "") {
            ...
            switch ($type) {
                ...
                case 'square':
                    ...
                    $html .= '<i class="qode_icon_element fa '.$icon.'" style="'.$style.'"></i>';
                    break;
                default:
        ...
        if($image == "") {
            ...
            switch ($icon_type) {
                case 'circle':
                    ...
                    $html_icon .= '<i class="qode_iwt_icon_element fa '.$icon.' fa-stack-1x" style="'.$style. '"></i>';
                    $html_icon .= '</span>';
                    break;
                case 'square':
                    $html_icon .= '<span '.qode_get_inline_attr($icon_type, 'data-icon-type').'  '.qode_get_inline_attr($icon_hover_color, 'data-icon-hover-color').' '.qode_get_inline_attr($icon_hover_background_color, 'data-icon-hover-bg-color').' class="qode_iwt_icon_holder fa-stack '.$icon_size.' '.$icon_stack_classes.'" style="'.$icon_stack_style.$icon_stack_square_style.'">';
                    $html_icon .= '<i class="qode_iwt_icon_element fa '.$icon.'" style="'.$style.'"></i>';
                    $html_icon .= '</span>';
                    break;
                default:
...
if (!function_exists('portfolio_list')) {
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if($type != 'masonry') {
            ...
            $html .= "<div class='projects_holder clearfix v$columns$_type_class $thumb_size_class $portfolio_loading_class'>\n";
            ...
            $html .= "<div class='projects_masonry_holder ". $grid_number_of_columns ." ".$portfolio_loading_class."'>";
            if (get_query_var('paged')) {
                $paged = get_query_var('paged');
            } elseif (get_query_var('page')) {
                $paged = get_query_var('page');
        ...
        if ($slider != "") {
            ...
            if ( have_posts() ) : $postCount = 0; while ( have_posts() ) : the_post();
                ...
                if(get_post_meta(get_the_ID(), "qode_slide-text-text-transform", true) != ""){
                    $text_text_transform = "text-transform: ". get_post_meta(get_the_ID(), "qode_slide-text-text-transform", true) . ";";
                }
...
//				if(get_post_meta(get_the_ID(), "qode_slide-text-field-type", true) == "text" && get_post_meta(get_the_ID(), "qode_slide-text", true) != ""){
                $html_text .= '<p class="q_slide_text" style="'.$text_color.$text_font_size.$text_line_height.$text_font_family.$text_font_style.$text_font_weight.$text_shadow.$text_text_transform.'" '.$slide_text_data.'><span>'.get_post_meta(get_the_ID(), "qode_slide-text", true).'</span></p>';
//               }
        ...
        if (have_posts()) : $postCount = 0;
            while (have_posts()) : the_post();
            	...
            	if ($type == "" || $type == "carousel"){
					...
					$html .= '<div class = "blog_image_holder">';
					$html .= '<span class = "image">' .get_the_post_thumbnail(get_the_ID(),'full'). '</span>';
