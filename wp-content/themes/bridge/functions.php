...
if(!function_exists('qode_google_fonts_styles')) {
	...
	function qode_google_fonts_styles() {
		...
		$font_weight_str 		= '100,200,300,400,500,600,700,800,900,400italic';
...
if(!function_exists('qode_disable_yoast_page_analysis')){
    /**
     * Function that disable yoast page analysis
     * @return bool
     */
    function qode_disable_yoast_page_analysis($bool){
    ...
    add_filter( 'wpseo_use_page_analysis', 'qode_disable_yoast_page_analysis' );
}

if(!function_exists('qode_yoast_page_analysis_notice')) {
    /**
     * Prints admin notice for Yoast page analysis if plugin is installed
     *
     * @see qode_admin_notice
     */
    function qode_yoast_page_analysis_notice() {
        if(qode_seo_plugin_installed()) {
            qode_admin_notice(
                'yoast_page_analysis_notice',
                __('Yoast SEO page analysis functionality has been disabled due to coding issue in plugin. Plugin author has been notifed and we hope that this issue will be resolved soon.', 'qode'),
                'updated',
                true
            );
        }
    ...
    add_action('admin_notices', 'qode_yoast_page_analysis_notice');
}
